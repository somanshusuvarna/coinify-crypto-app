...
print("✅ SYSTEM CHECK: Loading Coinify...")
...
def get_market_data():
    ...
    try:
        ...
        for symbol, ticker in tickers.items():
            ...
            # Dynamic Assets
            logo_url = f"https://raw.githubusercontent.com/spothq/cryptocurrency-icons/master/128/color/{name.lower()}.png"
            sparkline_url = f"https://www.coingecko.com/coins/{rank}/sparkline.svg"
...
if st.session_state.selected_asset is None:
    ...
    if not df.empty:
        ...
        btc_data = df.loc[df['Name'] == 'BTC'].iloc[0]
        ...
        with m1:
            with st.container(border=True):
                ...
                c_metric.metric(top_gainer['Name'], f"${top_gainer['Price']:,.4f}", f"+{top_gainer['Change']:.2f}%")
        ...
        with m2:
            with st.container(border=True):
                ...
                c_metric.metric(top_loser['Name'], f"${top_loser['Price']:,.4f}", f"{top_loser['Change']:.2f}%")
        ...
        with m3:
            with st.container(border=True):
                ...
                c_metric.metric("Bitcoin", f"${btc_data['Price']:,.2f}", f"{btc_data['Change']:.2f}%")
    ...
    with tab1:
        with st.spinner("Loading History..."):
            ...
            if last['close'] < last['lower']:
                m2.metric("Signal", "BUY ZONE", "Oversold")
            elif last['close'] > last['upper']:
                m2.metric("Signal", "SELL ZONE", "Overbought")
            else:
                m2.metric("Signal", "NEUTRAL", "Hold")
